...
function addSquare(color) {
    var size = parseInt(document.getElementById('scaleInput').value);
...
// Function to add subsequent squares with midpoints at the corners of existing squares
function addSquareTextInput(scale, color) {
    var size = scale * 100; // Scale up the size
    const existingSquares = [...squares]; // Create a copy of squares array to avoid adding new squares during loop
    existingSquares.forEach(existingSquare => {
        // Calculate the coordinates of the corners of the existing square
        const topLeft = { x: existingSquare.x, y: existingSquare.y };
...
function handleSubmit() {
    const red = parseInt(document.getElementById('RInput').value);
    const green = parseInt(document.getElementById('GInput').value);
    const blue = parseInt(document.getElementById('BInput').value);
    const color = `rgb(${red}, ${green}, ${blue})`;
    
    // Normalize the scale input
    let scale = parseInt(document.getElementById('scaleInput').value);
    ...
    if (squares.length === 0) {
        ...
        addSquareTextInput(scale, color);
        removeSquares();
